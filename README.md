...
├── poin.js         # rekap poin siswa (sumber: aplikasi Jurnal Mengajar — update manual)
...
## Rekap Poin (dari aplikasi Jurnal Mengajar)

`poin.js` berisi rata-rata poin tiap siswa, terpisah dari `data.js` supaya
mudah diperbarui manual tanpa menyentuh data induk. Isinya tampil sebagai:
...
nilai default 0–7 sesuai poin per pertemuan di Jurnal Mengajar.
...
- **Rekap Poin** — kolom poin per siswa (dari aplikasi Jurnal Mengajar) di
  tabel detail kelas, plus badge rata-rata poin di tiap kartu kelas
  (lihat bagian Rekap Poin di atas)
